...
import { BadgeIndianRupee, BookOpenText, ChevronDown, ChevronUp, Clock,X, ImageIcon, ListOrdered, PenLine, Plus, Star, Upload, UserPen, Video  } from 'lucide-react';
const Addpage = () => {
...
const computeCourseTotals = (lectures = []) => {
 ...
 const submitToBackend = async () => {
  ...
  try {
    ...
    fd.append("totalLectures",String(formData.totalLectures || computed.totalLectures || 0));

    // ── price — dot-notation keys (FIX: was a single JSON string) ──────────
    // FormData cannot send nested objects; dot-notation is the standard
    // pattern. The controller reads body["price.original"] and body["price.sale"].
    fd.append("price.original", String(Number(formData.price?.original) || 0));
    fd.append("price.sale",     String(Number(formData.price?.sale)     || 0));

    // ── totalDuration — dot-notation keys (same fix) ───────────────────────
    fd.append("totalDuration.hours",   String(computed.totalDuration?.hours   || 0));
    fd.append("totalDuration.minutes", String(computed.totalDuration?.minutes || 0));

    // ── lectures — must stay as JSON (arrays have no other FormData option) ─
    ...
    if (!res.ok) {
      const message = data?.message || data?.error || "Failed to create course";
      toast.error(message);
      setLoading(false);
      return;
    }
    ...
    <Toaster position="top-right" toastoptions={{duration: 3000}} />
      <div className={addPageStyles.contentContainer}>
            <div className={addPageStyles.headerContainer}>
                <div className={addPageStyles.headerGradient}></div>
                <div className={addPageStyles.headerGlow}></div>
                <h1 className={addPageStyles.headerTitle}>create a new course</h1>
            </div>
            <p className={addPageStyles.headerSubtitle}>craft an exception learning experience with our intuitive course creation tools</p>
      </div>
